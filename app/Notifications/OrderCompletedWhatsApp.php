...
class OrderCompletedWhatsApp extends BaseNotification
{
    ...
    public function toWhatsApp($notifiable)
    {
        ...
        $content = $clientName. PHP_EOL. PHP_EOL .__('email.order.line1').$this->invoice_number.PHP_EOL. PHP_EOL .__('email.order.line2').date('j F Y g:ia',strtotime($this->order->updated_at)).__('email.order.line3').PHP_EOL.PHP_EOL.__('email.order.line4').$this->order->total.__('email.order.line5').PHP_EOL.PHP_EOL.__('email.order.line6').$this->company->company_phone.__('email.order.phone_symbole').PHP_EOL.PHP_EOL.__('email.order.line7').PHP_EOL.PHP_EOL.__('email.order.line8').'Best Regards, '.PHP_EOL.PHP_EOL.$this->company->company_name.PHP_EOL.PHP_EOL.$this->company->company_phone;
